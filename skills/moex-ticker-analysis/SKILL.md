...
### Поиск новостей: двухуровневый подход

**Принцип:** RSS для свежего (быстро), web для глубокого (качественно).

#### Уровень 1: RSS (свежие/горячие)

Быстрый поиск по RSS-лентам за последние дни. Используется для оценки текущего новостного фона.

```bash
./vendor/bin/news news:search "{тикер или название}" --limit=20 --format=json
```

| Когда использовать | Что даёт |
|--------------------|----------|
| Быстрый анализ     | Свежие новости за 1-3 дня |
| Горячие события    | Срочные публикации |

**Если RSS пуст или недостаточно** → переход на Уровень 2.

#### Уровень 2: news-web (исторические/глубокие)

Каскадный поиск по новостным сайтам (Interfax, Kommersant, PRIME, SmartLab). Используется для фундаментального анализа.

**Использовать skill `news-web`** — там описан полный алгоритм каскадного поиска.

| Когда использовать | Что даёт |
|--------------------|----------|
| Компании 2-го эшелона | Новости, которых нет в RSS |
| Глубокий анализ | Исторический контекст |
| Поиск риск-факторов | События за 3-6 месяцев |

**Алгоритм:**

```
1. RSS news:search → если есть значимые новости: СТОП
2. Если RSS пуст → news-web skill (каскадный поиск)
```

**Связанный skill:** `news-web` — детальное описание источников и алгоритма поиска.
...
- RSS: [свежие за 1-3 дня]
- Web (если требуется): [исторические/глубокие]
